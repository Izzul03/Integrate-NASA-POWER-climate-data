...
def load_nasa_climate():
    """
    Load the NASA POWER climate CSV generated by app_nasa.py.
    Looks in the same folder as app.py first, then a few fallback paths.
    Returns a DataFrame with columns [state, year, mean_temp_c, mean_humidity_pct]
...
def load_data():
    ...
    # ── 4. Clean production / planted_area ──────────────────────────────
    for col in ["production", "planted_area"]:
        if col in df.columns:
            df[col] = clean_numeric(df[col])

    # ── 5. Drop old temperature/humidity columns from crops CSV ─────────
    ...
    if nasa is not None:
        # Rename NASA columns → dashboard column names
        ...
        # No NASA file found — add empty columns so the rest of the app doesn't break
        df["temperature"] = np.nan
        df["humidity"] = np.nan
        st.sidebar.error(
            "❌ climate_state_2017_2022.csv not found. "
            "Run app_nasa.py first, or upload the file above."
        )
...
# FIX: Check if min_year and max_year are equal to prevent slider API exception
...
# =============================================================================
    ...
    # FIX: Run timeline visuals only if multiple years exist; otherwise, provide a fallback view
    if len(time_data) > 1:
        st.altair_chart(
        ...
        # Fallback block when dataset only yields 1 year
        st.info(
    ...
    with col1:
        if len(scatter) >= 2:
            ...
            st.caption("Not enough records to compute Temperature vs Yield distributions.")
    ...
    with col2:
        if len(scatter_h) >= 2:
            ...
            st.caption("Not enough records to compute Humidity vs Yield distributions.")
    ...
    if sens_rows:
        ...
        st.warning(
            "⚠️ Linear calculations require numerical climate variations. Try expanding your selections across multiple states or years.")
